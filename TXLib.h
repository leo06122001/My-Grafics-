//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2020-05-15 02:04:56 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 166, 2020-05-15 02:04:56 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 166, 2020-05-15 02:04:56 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 166, 2020-05-15 02:04:56 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//-----------------------------------------------------------------------------------------------------------------
    ...
    #define _wsplitpath_s( path, drive, szDrive,     \
                           dir, szDir, name, szName, \
                           ext, szExt )                  _wsplitpath ( path, drive, dir, fname, ext )
#endif
...
#define     tx_printfy( formatArgNum )           __attribute__ (( format (printf, (formatArgNum), (formatArgNum)+1) ))
#define     tx_scanfy(  formatArgNum )           __attribute__ (( format (scanf,  (formatArgNum), (formatArgNum)+1) ))
...
//!          Не надо помещать в TX_ASSERT() действия, которые важны для работы алгоритма!
//!
...
bool             _txCreateMiniDump           (EXCEPTION_POINTERS* exc);
...
int                          _txOGLError                 = 0;          // Last OpenGL error when using tx_glGetError()
...
$   bool ok = _TX_CALL (Win32::MiniDumpWriteDump, (GetCurrentProcess(), GetCurrentProcessId(), file,
                                                   (Win32::MINIDUMP_TYPE) (Win32::MiniDumpWithIndirectlyReferencedMemory | Win32::MiniDumpScanMemory),
                                                   &excInfo, NULL, NULL));
$   CloseHandle (file);
...
struct _txDumpVarSuffix
    ...
    inline  explicit _txDumpVarSuffix (const char suffix[] = "") : suffix_ (suffix) { assert (suffix); }
